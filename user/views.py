...
def custom_login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            # Kiểm tra xem người dùng có thuộc nhóm "Admin" hay không
            if username == 'admin' and password == 'Admin1234':
                if not user.groups.filter(name='Admin').exists():
                    admin_group, created = Group.objects.get_or_create(name='Admin')
                    user.groups.add(admin_group)
                return redirect('courses:new_home')  # Điều hướng đến trang new_home hoặc trang admin

            # Kiểm tra nếu user thuộc nhóm 'Viewer' (fsa hoặc teacher)
            elif username == 'fsa' and password == 'Fsa1234':
                if not user.groups.filter(name='Viewer').exists():
                    viewer_group, created = Group.objects.get_or_create(name='Viewer')
                    user.groups.add(viewer_group)
                return redirect('courses:list_courses')  # Điều hướng đến trang courses

            # Nếu không thuộc nhóm nào thì chuyển hướng về trang courses
            return redirect('courses:list_courses')
        else:
            # Nếu xác thực thất bại, trả về trang login với thông báo lỗi
